...
Minecraft Java Edition reverse proxy. Routes incoming connections to backend servers based on the hostname the client connected to (from the Handshake packet). Zero dependencies — pure Node.js `net` module.
...
- After the handshake is parsed, raw `socket.pipe(socket)` is used — Node's fastest path for TCP splicing
